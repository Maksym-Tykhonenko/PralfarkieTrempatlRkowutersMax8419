...
import { LoadingShrine } from './entryGlow/LoadingShrine';
import { OnboardingFlow } from './sacredSlides/OnboardingFlow';
import { JourneyShell } from './journeyShell/JourneyShell';
...
//type Phase = 'loading' | 'onboarding' | 'journey';

export const ShrineRoot = () => {
...
useEffect(() => {
  if (timeStampUserId) {
    console.log('OneSignal.login із таймштампом:', timeStampUserId, 'полетів');
...
useEffect(() => {
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <Stack.Navigator
        ...
        <Stack.Screen name="Home" component={ShrineRootNavigation} />
      </Stack.Navigator>
    );
  };
  ...
  return (
    <NavigationContainer>
      ...
      {!isLoading ? <LoadingShrine /> : <Route isFatch={route} />}
    </NavigationContainer>
  )
};

const ShrineRootNavigation = () => { 
const [phase, setPhase] = useState('onboarding');
  const [onboardingComplete, setOnboardingComplete] = useState(null);

  useEffect(() => {
    vaultOnboarding.read().then(setOnboardingComplete);
  }, []);

  const handleLoadingDone = () => {
    if (onboardingComplete === null) {
      setTimeout(handleLoadingDone, 120);
      return;
    }
    setPhase(onboardingComplete ? 'onboarding' : 'onboarding');
  };
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={styles.host}>
        {phase === 'loading' && <LoadingShrine onDone={handleLoadingDone} />}
        {phase === 'onboarding' && <OnboardingFlow onFinish={handleOnboardingDone} />}
        {phase === 'journey' && <JourneyShell />}
      </View>
    </SafeAreaProvider>
  );
